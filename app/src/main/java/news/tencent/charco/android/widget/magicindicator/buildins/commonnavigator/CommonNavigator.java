package news.tencent.charco.android.widget.magicindicator.buildins.commonnavigator;
...
import news.tencent.charco.android.widget.magicindicator.NavigatorHelper;
import news.tencent.charco.android.widget.magicindicator.ScrollState;
import news.tencent.charco.android.widget.magicindicator.abs.IPagerNavigator;
import news.tencent.charco.android.widget.magicindicator.buildins.commonnavigator.abs.CommonNavigatorAdapter;
import news.tencent.charco.android.widget.magicindicator.buildins.commonnavigator.abs.IMeasurablePagerTitleView;
...
import news.tencent.charco.android.widget.magicindicator.buildins.commonnavigator.model.PositionData;
        ...
        if (mAdapter != null) {
            ...
            if (mScrollView != null && mPositionDataList.size() > 0 && position > 0 && position < mPositionDataList.size()) {
                if (mFollowTouch) {
                    ...
                    int lastposition = Math.min(mPositionDataList.size() - 1, position - 1);
                    PositionData current = mPositionDataList.get(currentPosition);
                    PositionData next = mPositionDataList.get(nextPosition);
                    PositionData last = mPositionDataList.get(lastposition);
                    int scrollX = mScrollView.getScrollX();
//                    int myScrollX = mScrollView.getMyScrollX();
                    int width = mScrollView.getWidth();

                    if (next.mRight > scrollX + width){
                        mScrollView.scrollTo(scrollX + next.contentWidth() ,0);
                    }else {
                        if (last.mLeft < scrollX){

                            mScrollView.scrollTo(scrollX - last.contentWidth() ,0);
                        }
                    }
